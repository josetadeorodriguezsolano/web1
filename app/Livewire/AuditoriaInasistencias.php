    ...
    public function exportarPDF()
{
    $audits = Audit::with('user')
        ->where('auditable_type', 'App\Models\Inasistencia')
        ->when($this->eventoSeleccionado, fn($q) => $q->where('event', $this->eventoSeleccionado))
        ->when($this->matricula, function ($query) {
            $alumno = Alumno::where('matricula', $this->matricula)->first();
            if ($alumno) {
                $query->where(function ($q) use ($alumno) {
        ...
        ->when($this->materiaId, fn($q) => $q->where('new_values->materia_id', $this->materiaId)
                                            ->orWhere('old_values->materia_id', $this->materiaId))
        ->when($this->mesSeleccionado !== 'alltime', fn($q) => $q->whereMonth('created_at', $this->mesSeleccionado))
        ->when($this->maestroId, fn($q) => $q->where('user_id', $this->maestroId))
        ->orderBy('created_at', 'desc')
        ->get();

    $pdf = Pdf::loadView('pdf.auditoria-inasistencias', [
        'audits' => $audits
    ]);

    return response()->streamDownload(function () use ($pdf) {
        echo $pdf->output();
    }, 'auditoria-inasistencias.pdf');
}
